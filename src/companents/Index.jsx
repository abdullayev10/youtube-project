...
export default function Index() {
    ...
    useEffect(() => {
        axios.get("https://youtube-backend-pvou.onrender.com/api/users").then(res => {
    ...
    useEffect(() => {
        axios.get(`https://youtube-backend-pvou.onrender.com/api/files?fileName=${fileName}`).then(res => {
            // console.log(res)
    ...
    formData.append("fileNomi", fileName)

    function handlClick(e) {
        e.preventDefault()

        axios.post(`https://youtube-backend-1-iuap.onrender.com/api/users${data.userId}`, formData, {
        }).then(res => {
            setDatas(res.data.data)
        ...
        <div class="container2">
            <header class="header" >
                ...
                <form action="#" class="search-box">
                    ...
                    <button class="search-btn hover-after" style={{ "background": "none", "border": "none", "margin-left": "10px", "width": "20px" }}>
                        <img src={img1} alt="mic" width="20" height="25" />
                    </button>
                </form>
                <section class="header-right">
                    ...
                    <Link to={token ? "/admin" : "/"}>
                        <img class="avatar-img"
                            src={`${API}/` + data.profilImg}
                            alt="avatar-img"
                            width="32px"
                            height="32px" onClick={handlClick} />
                    </Link>
            ...
            <main class="main">
                <div class="left-menu-wrapper">
                    <ul class="navbar-list">
                        ...
                        </li>
                        {
                            ...
                            datas.map(el => {
                                return <li class="channel" data-id={data.id}>
                                    <a href="#">

                                        <img src={"https://youtube-backend-pvou.onrender.com/" + el.profilImg} alt="channel-icon" width="30px" height="30px" />
                ...
                <div class="iframes-wrapper">
                    <ul class="iframes-list">
                        {
                            files.map(file => {
                                let hour = new Date(file.date).getHours();
                                let minutes = new Date(file.date).getMinutes();
                                let second = new Date(file.date).getSeconds();
                                let year = new Date(file.date).getFullYear();
                                let months = new Date(file.date).getMonth();
                                let day = new Date(file.date).getDay();
                                return <li class="iframe">
                                    <object style={{ "width": "350px", "height": "200px" }} data={"https://youtube-backend-pvou.onrender.com/" + file.file} controls=""></object>
                                    <div class="iframe-footer">
                                        <img src={"https://youtube-backend-pvou.onrender.com/" + file.user.profilImg} />
                                        <div class="iframe-footer-text">
                                            <h2 class="channel-name">{file.user.userName}</h2>
                                            <h3 class="iframe-title">{file.title}</h3>
                                            <time class="uploaded-time">{`${year}/${months}/${day} | ${hour}:${minutes}:${second}`}</time>
                                            <a class="download" href={`${API}/api/files/dowload/${file.file}`}>
                                                <span>{Math.floor(file.size * 10)}</span>
                                                <img src={img3} />
                                            </a>
                                        </div>
                                    </div>
